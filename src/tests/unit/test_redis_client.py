...
def test_redis_client_init():
    ...
    client = RedisClient(logger, host="localhost", port=6379, username="default", password="default", db=0)
...
def test_redis_client_connect_success():
    ...
    with patch("src.cache.client.redis.StrictRedis") as mock_redis:
        ...
        client = RedisClient(logger, host="localhost", port=6379, username="default", password="default", db=0)
        client.connect()
        mock_redis.assert_called_once_with(host="localhost", port=6379, username="default", password="default", db=0)
